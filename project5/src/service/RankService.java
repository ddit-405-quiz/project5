...
public class RankService {
	...
	public int infinityRankingPage() {
		
		List<Map<String, Object>> rankList = rankDAO.getInfinityRank();
		
		PrintUtil.bar();
		System.out.println();
		PrintUtil.centerAlignment("\tR  A  N  K  I  N  G");
		PrintUtil.bar2();
		PrintUtil.centerAlignment("\t\t    순위                           유저                         최고점수");
		System.out.println();
		
		for (Map<String, Object> ranking : rankList) {
			int rank = Integer.parseInt(ranking.get("RANK").toString());
			String ranker = (String) ranking.get("USER_NAME");
			int score = Integer.parseInt(ranking.get("USER_INFINITY").toString());
			System.out.println("\t\t  " + rank + "\t\t" + ranker + "\t     " + score);
		}
